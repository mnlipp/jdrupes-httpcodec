...
/**
 * A base class for converters for values with parameters. 
 * Converts field values such as `value; param1=value1; param2=value2`.
 * 
 * This class is actually a {@link Converter}, but does not implement
 * `Converter<ParameterizedValue<T>>` to avoid conflicts with
 * derived types that want to implement `Converter` with a different
 * type paramter `T`. {@link ParameterizedValueConverter} provides
 * a concrete converter for `Converter<ParameterizedValue<T>>`.
 * 
 * @param <T> the type of the value
 */
public class AbstractParamValueConverter<T> {
	 ...
	 */
	public AbstractParamValueConverter(Converter<T> valueConverter) {
